...
!! 10 column: semi minor axis of L plane cross section with shell
               ...
               read (23,*) table_rel_r(j,l), xi_arr_dummy, table_grav(j,l),&
                   A_shell_dummy, g_Lplane_arr_dummy
            end do
            close (23)
...
!!! locally used variables
         ...
         real*8 grav_final
         real*8 :: q_local, q_total, r_rel_loc, q_lim
         real*8 :: r_lin, r_lout
...
!!!!!!!!!!!!!! the following equations are from the article, equation 23
         ...
         if(flag_accel.eq.1) then
            ...
            effective_gravity = 1.d0+(-2.d0*(1.d0+q_total)*(r_rel_loc**3.d0)/(3.d0*q_local))
...
!! return pre-assigned value as in a single star             
         ...
         do j =1,n_q-1
            if (q_total.ge.q_table(j).and.&
               ...
               j_found=j
               ! write (*,*) "q_total=", q_total
               ! write (*,*) "q_table(j)=", q_table(j)
               ! write (*,*) "q_table(j+1)=", q_table(j+1)
               r_lin  = table_rel_r(j_found,500)
               r_lout  = table_rel_r(j_found,600)

               exit
            end if
         end do

!!!!!!!!!! if star has mass profile less steep than constant density, the code stops
         q_lim = q_total*(r_rel_loc/r_lin)**3.d0
         ! write (*,*) "q_lim = ", q_lim 
         ! write (*,*) "j found = ", j_found
         ! write (*,*) "r_lin = ", r_lin

         if (q_local.lt.q_lim) then
            write (*,*) "error, non physical input, density should decrease outwards" 
            stop
         end if

!!!!!!!!! if relative radius is more than the first lagrange point, 
!!!!!!!!! code automatically assumes q_loc = 1, 
!!!!!!!!! (meaning the star isn't larger than the roche lobe size)
        if(r_rel_loc.gt.r_lin) then
            write(*,*) "star radius greater than Roche Lobe size, local mass coordinate set to be equal to donor star mass"
            q_local =q_total
            M_loc = M1
...
!! return pre-assigned value as in a single star     
         ...
         if (r_rel_loc.ge.table_rel_r(j_found,n_gtable)) then
            write (*,*)" Stars exceeds outer L equivalent volume, check input", r_rel_loc, table_rel_r(j_found,n_gtable)
            stop
         end if
         ...
         do k=1,2  ! k is 1 and 2             
            do l=1,(n_gtable-1)                
               if (r_rel_loc.ge.&
                  ...
                  ! write(*,*) "table_rel_r", table_rel_r(k+j_found-1,l)
                  ! write(*,*) "table_rel_r", table_rel_r(k+j_found-1,l+1)
                  ! write(*,*) "radius", r_rel_loc
         ...
         ! write(*,*) "grav_found", grav_found(1)
         ! write(*,*) "grav_found", grav_found(2)
         ! write(*,*) "grav_final", grav_final
                             
         if(flag_use_analyt.eq.0) then

            if(flag_accel.eq.1) then
               effective_gravity=GT_accel*(M_loc)*(grav_final+g_exterior)*q_total/(q_local*r_loc**2.d0)
            else
               effective_gravity=(grav_final+g_exterior)*q_total/q_local
            end if
