...
!!! dummy properties are those getting read from the tables        
         real*8 :: xi_arr_dummy
         real*8 :: A_shell_dummy, g_Lplane_arr_dummy
...
! find input mass ratio's corresponding rlin and rlout   
         ...
         do j =1,n_q-1
            if (q_input.ge.q_table(j).and.&
               ...
               ! write(*,*) "inner lagrange volume equivalent radius =",r_lin_val
               ! write(*,*) "1. outer lagrange vol radius =",r_lout_val
...
!!! locally used variables
         ...
         !!scaling the input
         r_rel_loc=r_loc/a
         ! write(*,*)"r_rel_loc", r_rel_loc
         q_local=M_loc/M2
         q_total=M1/M2  
         ! write(*,*) "qlocal", q_local
...
! find input mass ratio's corresponding rlin and rlout   
         ...
         do j =1,n_q-1
            if (q_total.ge.q_table(j).and.&
               ...
               ! write(*,*) "found_index", found_index
               ! write(*,*) "called for mass ratio =",q_total
               ! write(*,*) "inner lagrange volume equivalent radius =",r_lin_val
               ! write(*,*) "rloc =", r_loc
               ! write(*,*) "outer lagrange volume equivalent radius =",r_lout_val
               ! write(*,*) "m1/m2 =",(M1/M2)
               ! write(*,*) "rloc/rout**3 =",(r_rel_loc/r_lout_val)**3.d0

               ! write(*,*) "density limit qloc =",(M1/M2)*(r_rel_loc/r_lout_val)**3.d0
               exit
            end if
         end do  
...
!! return pre-assigned value as in a single star     
         ...
         if (r_rel_loc.gt.r_lout_val.or.abs(r_rel_loc-r_lout_val).le.0.000001d0) then
            write (*,*)" Stars exceeds L3 equivalent volume, check input", r_rel_loc !, table_rel_r(j_found,n_gtable) r_lout_val
            stop
         end if
         ...
         if(flag_use_analyt.eq.0) then
            ...
            ! write(*,*) "grav_final", effective_gravity
