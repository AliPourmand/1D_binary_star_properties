...
!! 10 column: semi minor axis of L plane cross section with shell
              ...
              read (23,*) xi_arr_dummy, table_rel_r(j,l), table_grav(j,l),&
...
!!!!!!!!!!!!!! the following equations are from the article, equation 23
         ...
         if(flag_accel.eq.1) then
            effective_gravity = GT_accel*(1.d0+(-2.d0*(1.d0+&
               q_total)*(r_rel_loc**3.d0)/(3.d0*q_total)))*M_loc/r_loc**2.d0
         else
            effective_gravity = 1.d0+(-2.d0*(1.d0+q_total)*(r_rel_loc**3.d0)/(3.d0*q_total))
